...
const sounds = [
  { name: 'Rain Drops', emoji: '🌧️', gradient: 'gradient-blue', color: 'text-blue-800', mood: 'Calming', type: 'rain' },
  { name: 'Ocean Waves', emoji: '🌊', gradient: 'gradient-lavender', color: 'text-purple-800', mood: 'Relaxing', type: 'ocean' },
  { name: 'Calm Piano', emoji: '🎹', gradient: 'gradient-peach', color: 'text-pink-800', mood: 'Peaceful', type: 'piano' },
  { name: 'Forest Birds', emoji: '🌲', gradient: 'gradient-green', color: 'text-green-800', mood: 'Grounding', type: 'forest' },
  { name: 'Thunderstorm', emoji: '⛈️', gradient: 'gradient-blue', color: 'text-blue-800', mood: 'Powerful', type: 'thunder' },
  { name: 'White Noise', emoji: '🌫️', gradient: 'gradient-cream', color: 'text-amber-800', mood: 'Focus', type: 'white' },
  { name: 'Coffee Shop', emoji: '☕', gradient: 'gradient-cream', color: 'text-amber-800', mood: 'Cozy', type: 'coffee' },
  { name: 'Fireplace', emoji: '🔥', gradient: 'gradient-peach', color: 'text-pink-800', mood: 'Warm', type: 'fire' },
  { name: 'Tibetan Bowls', emoji: '🎵', gradient: 'gradient-lavender', color: 'text-purple-800', mood: 'Meditative', type: 'bowls' },
  { name: 'Birds at Dawn', emoji: '🐦', gradient: 'gradient-green', color: 'text-green-800', mood: 'Uplifting', type: 'birds' },
  { name: 'Deep Space', emoji: '🌌', gradient: 'gradient-blue', color: 'text-blue-800', mood: 'Cosmic', type: 'space' },
  { name: 'Gentle Stream', emoji: '🏞️', gradient: 'gradient-green', color: 'text-green-800', mood: 'Serene', type: 'stream' },
  { name: 'Healing Frequencies', emoji: '🔮', gradient: 'gradient-lavender', color: 'text-purple-800', mood: 'Healing', type: 'healing' },
  { name: 'Soft Wind', emoji: '🍃', gradient: 'gradient-green', color: 'text-green-800', mood: 'Breezy', type: 'wind' },
  { name: 'Night Crickets', emoji: '🦗', gradient: 'gradient-blue', color: 'text-blue-800', mood: 'Soothing', type: 'crickets' },
];
...
function RainAnimation() {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden rounded-3xl pointer-events-none">
      {drops.map((i) => (
        <motion.div key={i} className="absolute rounded-full bg-blue-400/50"
          style={{ width: '2px', height: `${6 + (i % 4) * 3}px`, left: `${(i * 8) % 90 + 5}%`, top: '-10px' }}
          animate={{ y: [0, 130], opacity: [0, 0.8, 0] }}
          transition={{ duration: 0.8 + (i % 3) * 0.3, repeat: Infinity, delay: (i * 0.15) % 1.2, ease: 'linear' }}
        />
      ))}
    </div>
  );
    ...
    <div className="absolute inset-0 overflow-hidden rounded-3xl pointer-events-none">
      {particles.map((i) => (
        <motion.div key={i} className="absolute rounded-full"
          style={{ width: `${6 + (i % 4) * 4}px`, height: `${6 + (i % 4) * 4}px`, background: i % 3 === 0 ? 'rgba(251,146,60,0.7)' : i % 3 === 1 ? 'rgba(239,68,68,0.6)' : 'rgba(253,224,71,0.6)', left: `${20 + (i * 7) % 60}%`, bottom: '5px' }}
          animate={{ y: [0, -(40 + (i % 3) * 20)], x: [0, (i % 2 === 0 ? 6 : -6)], opacity: [0.8, 0], scale: [1, 0.3] }}
          transition={{ duration: 0.9 + (i % 3) * 0.3, repeat: Infinity, delay: (i * 0.12) % 1, ease: 'easeOut' }}
        />
      ))}
    </div>
  );
...
function OceanAnimation() {
  return (
    <div className="absolute inset-0 overflow-hidden rounded-3xl pointer-events-none">
      {[0, 1, 2].map((i) => (
        <motion.div key={i} className="absolute left-0 right-0 rounded-full"
          style={{ height: '3px', background: 'rgba(139,92,246,0.35)', bottom: `${10 + i * 12}px` }}
...
function ThunderAnimation() {
  return (
    <div className="absolute inset-0 overflow-hidden rounded-3xl pointer-events-none">
      <motion.div className="absolute inset-0 rounded-3xl" style={{ background: 'rgba(250,250,100,0.25)' }}
      ...
      />
      <motion.div className="absolute" style={{ left: '40%', top: '5px', fontSize: '20px' }}
...
function WhiteNoiseAnimation() {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden rounded-3xl pointer-events-none">
      {dots.map((i) => (
        <motion.div key={i} className="absolute rounded-full bg-amber-400/40"
          style={{ width: `${2 + (i % 3)}px`, height: `${2 + (i % 3)}px`, left: `${(i * 6) % 90 + 5}%`, top: `${(i * 11) % 80 + 5}%` }}
...
function BowlsAnimation() {
  return (
    <div className="absolute inset-0 overflow-hidden rounded-3xl pointer-events-none flex items-center justify-center">
      {[0, 1, 2].map((i) => (
        <motion.div key={i} className="absolute rounded-full border border-purple-400/40"
...
function SpaceAnimation() {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden rounded-3xl pointer-events-none">
      {stars.map((i) => (
        <motion.div key={i} className="absolute rounded-full bg-blue-200"
          style={{ width: `${1 + (i % 3)}px`, height: `${1 + (i % 3)}px`, left: `${(i * 9) % 90 + 5}%`, top: `${(i * 13) % 80 + 5}%` }}
          animate={{ opacity: [0, 1, 0], scale: [0.5, 1.5, 0.5] }}
...
function StreamAnimation() {
  return (
    <div className="absolute inset-0 overflow-hidden rounded-3xl pointer-events-none">
      {[0, 1, 2, 3].map((i) => (
        <motion.div key={i} className="absolute rounded-full border border-green-400/40"
...
function PianoAnimation() {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden rounded-3xl pointer-events-none">
      {notes.map((note, i) => (
        <motion.span key={i} className="absolute text-pink-400/60 text-sm font-bold"
...
function HealingAnimation() {
  return (
    <div className="absolute inset-0 overflow-hidden rounded-3xl pointer-events-none flex items-center justify-center">
      {[0, 1, 2, 3].map((i) => (
        <motion.div key={i} className="absolute rounded-full border border-purple-500/30"
...
function WindAnimation() {
  return (
    <div className="absolute inset-0 overflow-hidden rounded-3xl pointer-events-none">
      {[0, 1, 2].map((i) => (
        <motion.div key={i} className="absolute rounded-full bg-green-300/30"
          style={{ height: '2px', width: `${30 + i * 15}px`, left: '-10px', top: `${20 + i * 20}%` }}
          animate={{ x: [0, 140], opacity: [0, 0.7, 0] }}
          transition={{ duration: 1.8 + i * 0.4, repeat: Infinity, delay: i * 0.5, ease: 'easeInOut' }}
        />
      ))}
      {['🍃', '🌿'].map((leaf, i) => (
        <motion.span key={i} className="absolute text-xs"
          style={{ top: `${30 + i * 25}%`, left: '-10px' }}
          animate={{ x: [0, 130], opacity: [0, 1, 0], rotate: [0, 360] }}
          transition={{ duration: 2.5 + i * 0.5, repeat: Infinity, delay: i * 1.2, ease: 'easeInOut' }}
        >{leaf}</motion.span>
      ))}
    </div>
  );
...
function CricketsAnimation() {
  const stars = Array.from({ length: 8 }, (_, i) => i);
  return (
    <div className="absolute inset-0 overflow-hidden rounded-3xl pointer-events-none">
      {stars.map((i) => (
        <motion.div key={i} className="absolute rounded-full bg-blue-300/50"
          style={{ width: `${1 + (i % 2)}px`, height: `${1 + (i % 2)}px`, left: `${(i * 12) % 85 + 5}%`, top: `${(i * 17) % 75 + 5}%` }}
          animate={{ opacity: [0, 1, 0] }}
          transition={{ duration: 0.5 + (i % 3) * 0.3, repeat: Infinity, delay: (i * 0.3) % 1.5 }}
        />
      ))}
      <motion.span className="absolute text-xs" style={{ bottom: '8px', right: '12px' }}
...
function SoundAnimation({ type }: { type: string }) {
  switch (type) {
    ...
    case 'crickets': return <CricketsAnimation />;
    default: return null;
  }
}

export default function WellnessSoundCards() {
